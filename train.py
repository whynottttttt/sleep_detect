...
def accuracy(y_pred, y_test): # 예측값(model output과 label값을 parameter로 불러옴
    y_pred_tag = torch.round(torch.sigmoid(y_pred)) # 예측값을 sigmoid를 통해 확률화 시킨 후 round를 통해 0또는 1로 변환 (0.5이하는 0, 초과는 1)

    correct_results_sum = (y_pred_tag == y_test).sum().float() # 맞춘 갯수를 count
    acc = correct_results_sum / y_test.shape[0]
    acc = torch.round(acc * 100) #전채 갯수만큼 나눈 후 100을 곱한 값을 return
...
def main():
    ...
    for epoch in range(epochs):
        ...
        model.train() # 학습 모드 선언 (안 해도 상관 없음)
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            loss = criterion(outputs, labels) # model의 output을 criterion을 통해 loss를 구함
            loss.backward()
            optimizer.step() # loss update
